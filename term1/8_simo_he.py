...
r_df = pd.read_csv('SimoneCode\stream_r_001.csv',header=None)
th_df = pd.read_csv('SimoneCode\stream_th_001.csv',header=None)
...
for i in range(len(stream_l)):
    ...
    f0 = np.array([1,1e-10])
    sol_f = ivp(get_rhs, l0, f0, method='LSODA', 
                args = [i], t_eval = stream_l[i]* rb[0],
