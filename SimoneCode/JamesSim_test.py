...
#%%
plt.plot(Radii*np.sin(Angles[13]),Radii*np.cos(Angles[13]))
...
for i in np.arange(30,35):
        x_R = sol_rad[5]*np.sin(sol_ang[i])
        y_R = sol_rad[5]*np.cos(sol_ang[i])
...
#%%
f1 = plt.figure
plt.contourf(X,Z,sign_vr)
c = plt.colorbar()
plt.xlim(1,15)

f2 = plt.figure()
plt.contourf(X,Z,sign_vth)
c = plt.colorbar()

#%%
## find functional form for starting radii
plt.plot(init_r,init_th,'.')
plt.plot(init_R,thb)
plt.plot(fine_r,np.tan((fine_r-1.025)/0.009)+1.4)
plt.plot(fine_r,14*np.sqrt(np.abs(fine_r-1.0165)))
plt.xlim(1.015,1.035)
plt.ylim(0,np.pi)
plt.show()
...
def Th_r_min(rad,a,b):
    '''
    Piecewise function for minimum radius to start temperature at
